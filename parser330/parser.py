...
class Parser(object):
    ...
    def __init__(self):
        ...
        # Users
        self.USERNAME_REGEX = re.compile(r'\B' + AT_SIGNS + LIST_END_CHARS, re.IGNORECASE)

        # Hashtags
        self.HASHTAG_EXP = r'(^|[^0-9A-Z&/]+)(#|\uff03)([0-9A-Z_]*[A-Z_]+[%s]*)' % UTF_CHARS
        self.HASHTAG_REGEX = re.compile(HASHTAG_EXP, re.IGNORECASE)


        # URLs
        self.PRE_CHARS = r'(?:[^/"\':!=]|^|\:)'
        self.DOMAIN_CHARS = r'([\.-]|[^\s_\!\.\/])+\.[a-z]{2,}(?::[0-9]+)?'
        self.PATH_CHARS = r'(?:[\.,]?[%s!\*\'\(\);:=\+\$/%s#\[\]\-_,~@])' % (UTF_CHARS, '%')
        self.QUERY_CHARS = r'[a-z0-9!\*\'\(\);:&=\+\$/%#\[\]\-_\.,~]'

        # Valid end-of-path chracters (so /foo. does not gobble the period).
        # 1. Allow ) for Wikipedia URLs.
        # 2. Allow =&# for empty URL parameters and other URL-join artifacts
        self.PATH_ENDING_CHARS = r'[%s\)=#/]' % UTF_CHARS
